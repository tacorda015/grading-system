$('.nav-link#updateBtn').on('click', function(e) {
    $.ajax({
        ...
        success: function (response) {
            ...
            // Add labels row
            var labelsRow = $('<div class="row"><div class="col">Session Name</div><div class="col">Grade Base</div><div class="col">Percentage</div></div>');
            ...
            gradingSessionData.forEach(function(session, index) {
                ...
                // Session Name
                var sessionNameCol = $('<div class="col"><div class="mb-3"><input type="text" class="form-control" name="updateSessionName[]" value="' + session.grading_session_name + '"></div></div>');
                row.append(sessionNameCol);

                // Grade Base
                var gradeBaseCol = $('<div class="col"><div class="mb-3"><input type="text" class="form-control" name="updateGradeBase[]" value="' + session.grading_session_base + '"></div></div>');
                row.append(gradeBaseCol);

                // Percentage
                var percentageCol = $('<div class="col"><div class="mb-3"><input type="text" class="form-control" name="updatePercentage[]" value="' + session.grading_session_percentage + '"></div></div>');
                row.append(percentageCol);

                var sessionId = $('<input type="hidden" name="updateSessionId[]" value="' + session.grading_session_id + '">');
                row.append(sessionId);
                // Append the row to the modal body
                modalBody.append(row);
            });
            // Add the hidden input only once after the loop
            var hiddenInput = $('<input type="hidden" name="updateCourseSubjectId" id="updateCourseSubjectId" value="'+ CourseSubjectIdSetted +'">');
...
function validateForm() {
    ...
    $('.modal-body .row.InputValidation').each(function () {
        ...
        if (!isValidInteger(gradeBase) || parseInt(gradeBase) > 100 || parseInt(gradeBase) < 0) {
            Swal.fire({
                toast: true,
                position: "top-end",
                icon: "error",
                title: "Grade Base should be a valid integer less than or equal to 100.",
                showConfirmButton: false,
                timer: 1500,
                timerProgressBar: true,
                animation:true,
        ...
        if (!isValidInteger(percentage) || parseInt(percentage) < 0 || parseInt(percentage) > 100) {
            Swal.fire({
                toast: true,
                position: "top-end",
                icon: "error",
    ...
    if (percentageTotal !== 100) {
        ...
        isValid = false;
        $('.modal-body .row.InputValidation [name^="updatePercentage"]').addClass('is-invalid');
    }
